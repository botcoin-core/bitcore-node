...
/**
 * Will return the path and default botcore-node configuration. It will search for the
 * configuration file in the "~/.botcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.botcore');
  var defaultConfigFile = path.resolve(defaultPath, './botcore-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    var now = new Date();
    // botcore-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `botcore-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
